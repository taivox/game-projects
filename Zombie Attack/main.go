...
func DrawState() {
	...
	PrintGameFrame()
	PrintGameObjects([]*GameObject{player})
...
func IsObjectOutOfBounds(obj *GameObject, velRow, velCol int) bool {
	for _, p := range obj.points {
		...
		if targetRow < 0 || targetRow >= GameFrameHeight || targetCol < 0 || targetCol >= GameFrameWidth {
			return true
		}
	}
	return false
...
func PrintGameFrame() {
	...
	PrintUnfilledRect(frameStartRow-1, frameStartCol-1, GameFrameWidth+2, GameFrameHeight+2, GameFrameSymbol)
}
